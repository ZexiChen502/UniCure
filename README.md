...
pip install numpy pandas scikit-learn fastparquet tqdm anndata scanpy lora-pytorch scipy geomloss
...
The `./Reproducibility` directory contains scripts for regenerating the paper-related analysis outputs and figure panels from the released raw data. For the complete data-to-result workflow, required inputs, and figure-by-figure instructions, see [`Reproducibility/UniCure_Paper_Results_Reproducibility_Guide.md`](Reproducibility/UniCure_Paper_Results_Reproducibility_Guide.md).
